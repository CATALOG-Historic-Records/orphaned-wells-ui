...
In source mode, Docker bind-mounts the backend source into the container and starts Uvicorn with `--reload`. Normal Python code edits are picked up by the running backend. Dependency, packaging, Dockerfile, and startup-command changes require `npm run docker:start` so Compose can rebuild and recreate the backend container.
