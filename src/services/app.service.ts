...
export const downloadRecords = (location: string, _id: string, export_types: { [key: string]: boolean }, output_name: string, data: any) => {
  let endpoint = `${BACKEND_URL}/download_records/${location}/${_id}?export_csv=${export_types["csv"] || false}&export_json=${export_types["json"] || false}&export_images=${export_types["image_files"] || false}&export_embedded_pdfs=${export_types["embedded_pdf_files"] || false}&reconstructed_pdfs=${export_types["reconstructed_pdf_files"] || false}&output_name=${output_name}`;
